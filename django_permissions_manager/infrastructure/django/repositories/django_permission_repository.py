...
class DjangoPermissionRepository(PermissionReadRepository):
    """Read-only Django implementation of PermissionReadRepository.

    Django generates permissions automatically through the migration system
    (post_migrate signal), so there is no need to create them programmatically.
    This adapter only exposes query operations.
    """
